...
class MessageGenerator
{
    ...
    public function greeting(): void
    {
        ...
        if (
            ($day === "Friday" && ($now > $sunset && $now > $soir))
            || ($day === "Saturday" && $now < $apresMidi)
        ) {
            $end = "Bon week-end";
        }
        if ($day === "Sunday" && $now < $midi) {
            $end = "Bon dimanche";
        }
        if ($day === "Monday" && $now > $midi && !($now < $sunset || $now < $soir)) {
            $end = "Bonne semaine";
        }
        if ((int)$now->format("m") === 12 && (int)$now->format("d") > 20) {
            $end = "Bonnes fêtes";
        }
        if ((int)$now->format("m") === 1 && (int)$now->format("d") < 10) {
            $end = "Bonne année";
        }
